...
// using template 'TriangleSoup2f. Do not modify this file directly, modify it from template source.
...
namespace SharpMedia.Math.Shapes.Storage
{
    ...
    /// </remarks>
    [Serializable]
    public class TriangleSoup2f : ITriangleAccess2f, ITriangleBuilder2f
    {
        ...
        public TriangleSoup2f(bool indexed)
        {
            if (indexed)
            {
        ...
        public Triangle2f Get(uint index)
        {
            int idx = (int)index;
            if (IsIndexed)
            {
                return new Triangle2f(vertices[(int)indices[idx * 3]], vertices[(int)indices[idx * 3 + 1]],
                                     vertices[(int)indices[idx * 3 + 2]]);
            }
            else
            {
                return new Triangle2f(vertices[idx * 3], vertices[idx * 3 + 1], vertices[idx * 3 + 2]);
            }
        }
        ...
        public void Get(uint index, Triangle2f[] triangles)
        {
        ...
        #region ITriangleBuilder2f Members
