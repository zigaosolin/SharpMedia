...
// using template 'TriangleSoup2d. Do not modify this file directly, modify it from template source.
...
namespace SharpMedia.Math.Shapes.Storage
{
    ...
    /// </remarks>
    [Serializable]
    public class TriangleSoup2d : ITriangleAccess2d, ITriangleBuilder2d
    {
        ...
        public TriangleSoup2d(bool indexed)
        {
            if (indexed)
            {
        ...
        public Triangle2d Get(uint index)
        {
            int idx = (int)index;
            if (IsIndexed)
            {
                return new Triangle2d(vertices[(int)indices[idx * 3]], vertices[(int)indices[idx * 3 + 1]],
                                     vertices[(int)indices[idx * 3 + 2]]);
            }
            else
            {
                return new Triangle2d(vertices[idx * 3], vertices[idx * 3 + 1], vertices[idx * 3 + 2]);
            }
        }
        ...
        public void Get(uint index, Triangle2d[] triangles)
        {
        ...
        #region ITriangleBuilder2d Members
