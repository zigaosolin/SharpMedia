...
// using template 'OutlineCompound2f. Do not modify this file directly, modify it from template source.
...
namespace SharpMedia.Math.Shapes.Compounds
{
    ...
    [Serializable]
    public sealed class OutlineCompound2f :
        IArea2f, IOutline2f, ITransformable2f, ICloneable<OutlineCompound2f>
    {
        ...
        /// </summary>
        private class ContourBuilder : Storage.Builders.ILineBuilder2f
        {
            #region Private Members
            List<Vector2f[]> contours = new List<Vector2f[]>();
            #endregion
            ...
            void FinishContour()
            {
                ...
                Vector2f[] lastCountour = contours[contours.Count - 1];
            ...
            #region Builders.ILineBuilder2f Members
            ...
            public void AddLineStrip(bool linkFirstToLast, params Vector2f[] data)
            {
                ...
                if (linkFirstToLast)
                {
                    ...
                    Vector2f[] newData = new Vector2f[data.Length + 1];
                ...
                else
                {
                    ...
                    if (contours.Count > 0)
                    {
                        Vector2f[] lastContour = contours[contours.Count - 1];
                        ...
                        if (lastContour != null && Vector2f.NearEqual(lastContour[lastContour.Length - 1], data[0]))
                        {
                            // We append data.
                            Vector2f[] newData = new Vector2f[data.Length + lastContour.Length];
                            int i;
                            ...
                            if (Vector2f.NearEqual(newData[0], newData[newData.Length - 1])) FinishContour();
        ...
        /// <returns></returns>
        public IOutline2f this[uint index]
        {
            get
            {
        ...
        public uint[] StopIndices
        {
            get
            {
                ...
                {
                    if (Vector2f.NearEqual(outlines[i].Sample(0.0f), outlines[i - 1].Sample(1.0f)))
                    {
                        stops.Add((uint)(i - 1));
                    }
                }
        ...
        public OutlineCompound2f(params IOutline2f[] outlines)
        {
        ...
        {
            if (obj is OutlineCompound2f)
            {
                return this.Equals((OutlineCompound2f)obj);
            }
            return false;
        }
        ...
        public void Transform(Matrix.Matrix3x3f matrix)
        {
            for (int i = 0; i < outlines.Length; i++)
            {
                if (!(outlines[i] is ITransformable2f))
                {
            ...
            for (int i = 0; i < outlines.Length; i++)
            {
                (outlines[i] as ITransformable2f).Transform(matrix);
            }
        }
        ...
        public OutlineCompound2f Clone()
        {
            return new OutlineCompound2f(outlines);
        }
    ...
    [TestSuite]
    internal class Test_OutlineCompound2f
    {
        [CorrectnessTest]
